...
export default function ManualAttendanceEntry() {
  ...
  const [item, setItem] = useState("Attendance DKP");
  const [itemId, setItemId] = useState(0);
  const [dkp, setDkp] = useState('');
  let {API, setLedgerData} = useContext(AppContext);
  const title = "Manually Add Attendance?";
  ...
  const actionButton = (update) => <Button onClick={() => postAddItem({character_name, dkp, itemId, item, raid_team})}>Confirm</Button>;
